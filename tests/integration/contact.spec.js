...
describe('Contact', () => {
  ...
  context('[POST] /api/v1/contact', () => {
    ...
    it('should be unauthorized if accessToken is not present', async () => {
      ...
      expect(response.body.message).to.equal('Authentication is required for this kind of operation.');
    });
    ...
    it('should receive bad request if wrong data input is sent', async () => {
      ...
      const response = await request(app)
        .post('/api/v1/contact')
        .send({ ...contact, firstName: undefined })
        .set('Accept', 'application/json')
     ...
     */
    it.skip.only('should create new contact', async () => {
      const token = await authOperations.signUp(user);
      await request(app)
        .post('/api/v1/contact')
        .send(contact)
